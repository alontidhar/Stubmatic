	...
	setTimeout(function(){
		...
		proxyModule.redirect(reqOptions, response, function( responseRecieved){
			if (options.recordAndPlay) {
				...
				let fileNamePrefix = options.recordFileNamePrefix ? options.recordFileNamePrefix.replace(/[^0-9a-zA-Z_-]/g, "") : null;
				fileNamePrefix = fileNamePrefix ? fileNamePrefix + "_" : "";
				let filePath = path.join(recordingPath, "_" + fileNamePrefix + fileDetail.name + "_" + String(Date.now()) );
